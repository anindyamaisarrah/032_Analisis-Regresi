...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\LENOVO\\OneDrive\\Documents\\Anreg\\final data\\Data Anreg_Anindya32.xlsx")
dt
```

```{r}
y <- dt$"Forest Area"
x <- dt$"Agricultural Land"
n <- length(dt)
```
...
Berdasarkan perhitungan, didapat dugaan persamaan regresi linear sebagai berikut. Dugaan persamaan tersebut dapat pula disebut sebagai nilai harapan dari peubah respons Y (Forest Area)

$$\hat{Y} = 49.85111 - 0.47224 X$$
...
Dari scatter plot di atas, Terlihat bahwa beberapa amatan tidak berada tepat pada garis dugaan regresi. Selisih antara nilai amatan $Y_i$ dengan nilai dugaan $\hat{Y}_i$ disebut sebagai galat (error). Keragaman dari galat tiap amatan ini dapat diurai berdasarkan garis dugaan persamaan ($\hat{Y}$) dan garis rataan nilai resposnya ($\bar{Y}$). Penguraian keragaman ini dapat dituliskan sebagai Jumlah Kuadrat Regresi (JKR), Jumlah Kuadrat Galat (JKG), dan Jumlah Kuadrat Total (JKT) dengan perhitungan sebagai berikut.

$$\text{JKR} = \sum_{i=1}^{n} (\hat{y}_i - \bar{y})^2 ; \text{JKG} = \sum_{i=1}^{n} (y_i - \hat{y}_i)^2 ; \text{JKT} = \sum_{i=1}^{n} (y_i - \bar{y})^2$$
Hubungan antara ketiganya dapat pula dituliskan sebagai berikut.

$$JKT = JKR + JKG$$
Nilai penguraian keragaman ini dapat diamati pula menggunakan fungsi anova pada bagian **Sum Sq (Jumlah Kuadrat)** seperti yang ditampilkan di bawah. Pada output sintaks di bawah, JKR model bernilai 21682 dan JKG model bernilai 87443

```{r}
(anova.model <- anova(model))
```

Dari tabel ANOVA dan penguraian keragaman, dapat diduga pula ragam galat/error dari model yang kita peroleh. Secara umum, error dari model dapat diduga oleh nilai Kuadrat Tengah Galat (KTG) sehingga berdasarkan output sintaks di atas, diketahui bahwa ragam galat dari model bernilai 467.6. Hubungan antara keduanya dapat dituliskan sebagai berikut.
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{467.6074} \approx 21.62423$
...
Nilai dugaan parameter β0 dan β1 sebesar 49.85111 dan -0.4722353. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Jika dalam uji hipotesis kita ingin menguji apakah $\beta_0$, maka nilai th (statistik uji) dapat dihitung sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [30.1201 ,  69.58213] dan untuk parameter $β_1$ adalah [-0.6419295 ,  -0.3025411]
...
$$ E(\hat{Y}|X=x_0) \pm t_{\alpha/2, n-2} S_e \sqrt{(\frac{1}{n} + \frac{(x_i - \bar{x})^2}{\sum_{i=1}^n(x_i-\bar{x})^2})}$$
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=10 adalah 45.12876. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang 40.30832 hingga 49.9492.
...
$$ \hat{y}(x_i)\pm t_{\alpha/2, n-2} S_e \sqrt{1+(\frac{1}{n} + \frac{(x_i - \bar{x})^2}{\sum_{i=1}^n(x_i-\bar{x})^2})}$$
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=10 adalah 45.12876. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang 2.198476 hingga 88.05905.
